...
static int smb328a_write_reg(struct i2c_client *client, int reg, u8 value)
{
	...
	pr_err("%s : Da_G i2c_smbus_write_byte_data reg %d, (%x), value %d, (%x)\n", __func__, reg, reg, value, value);
...
//	pr_err("%s : Da_G i2c_smbus_read_byte_data reg %d, (%x)\n", __func__, reg, reg);
	if (ret < 0) {
		pr_err("%s: err %d, try again!\n", __func__, ret);
		ret = i2c_smbus_read_byte_data(client, reg);
		if (ret < 0)
...
static void smb328a_set_command_reg(struct i2c_client *client)
{
	...
	if (val >= 0) {
		...
		if (chip->chg_mode == CHG_MODE_AC ||
			...
			data = 0xa9; /* usb */
			if ((chip->charge_current_override > 0) && (chip->charge_current_override < 1200)) {
				data = 0xad;
				pr_err("%s : Da_G Overriding charge mode to AC %d (%x)\n", __func__, chip->charge_current_override, chip->charge_current_override);
			}
			pr_err("%s : Da_G smb328a_write_reg 0xad = AC/MISC/Unk 0xa9 = USB %d, (%x), value %d, (%x)\n", __func__, reg, reg, data, data);
		if (smb328a_write_reg(client, reg, data) < 0)
			pr_err("%s : error!\n", __func__);
		val = smb328a_read_reg(client, reg);
		if (val >= 0) {
...
static void smb328a_charger_function_conrol(struct i2c_client *client)
{
	...
	if (val >= 0) {
		...
		if (chip->chg_mode == CHG_MODE_AC) {
			...
			pr_err("%s : Da_G charging current set to I717 fast 1A level %d, (%x)\n", __func__, set_data, set_data);
#else
			set_data = 0x97; /* fast 900mA */
			pr_err("%s : Da_G charging current set to I717 fast 900mA level %d, (%x)\n", __func__, set_data, set_data);
#endif
		} else if (chip->chg_mode == CHG_MODE_MISC) {
			set_data = 0x57; /* fast 700mA */
			pr_err("%s : Da_G charging current set to MISC fast 700mA level %d, (%x)\n", __func__, set_data, set_data);
		} else {
			if ((chip->charge_current_override > 0) && (chip->charge_current_override < 1201)) {
				set_data = 0xB7; /* fast 1A */
				chip->chg_mode = CHG_MODE_AC;
				pr_err("%s : Da_G charging current override to 1A level %d, (%x)\n", __func__, set_data, set_data);
			}
			else {
				set_data = 0x17; /* fast 500mA */
				pr_err("%s : Da_G charging current set to default fast 500mA level %d, (%x)\n", __func__, set_data, set_data);
			}
		}
	...
	if (val >= 0) {
		...
		if (chip->chg_mode == CHG_MODE_AC) {
			...
			pr_err("%s : Da_G charging current input set to AC mode 1A level %d, (%x)\n", __func__, set_data, set_data);
		}
		else if (chip->chg_mode == CHG_MODE_MISC) {
			set_data = 0x50; /* input 700mA */
			pr_err("%s : Da_G charging current input set to MISC mode 700mA level %d, (%x)\n", __func__, set_data, set_data);
		}
		else {
			if ((chip->charge_current_override > 0) && (chip->charge_current_override < 1201)) {
				set_data = 0xb0; /* input 1A */
				chip->chg_mode = CHG_MODE_AC;
				pr_err("%s : Da_G charging current input override to 1A level %d, (%x)\n", __func__, set_data, set_data);
			}
			else {
				set_data = 0x10; /* input 450mA */
				pr_err("%s : Da_G charging current input set to lowest mode 450mA level %d, (%x)\n", __func__, set_data, set_data);
			}
		}
	...
	set_data = 0xd5;
	pr_err("%s : Da_G override set_data by model %d, (%x)\n", __func__, set_data, set_data);

#else
	
#if defined(CONFIG_TARGET_LOCALE_USA)
		set_data = 0x4d;
	pr_err("%s : Da_G override set_data by locale %d, (%x)\n", __func__, set_data, set_data);
#else
		set_data = 0xd5;
	pr_err("%s : Da_G override set_data by nonmatch locale %d, (%x)\n", __func__, set_data, set_data);
#endif
#endif
		if (data != set_data) {
			data = set_data;
	...
	pr_err("%s : Da_G valueof charging set_data %d (%x)\n", __func__, set_data, set_data);
}
					...
					int chg_current)
{
	...
	pr_err("%s : Da_G setting charge current requested: %d (%x)\n", __func__, chg_current, chg_current);

	if (chg_current < 450 || chg_current > 1200)
		pr_err("%s : Da_G charging current outside bounds 450-1200 %d, (%x)\n", __func__, chg_current, chg_current);
		return -EINVAL;

	chip->chg_set_current = chg_current;
	
	pr_err("%s : Da_G AC=900+ USB=500 MISC=700 UNKNOWN=450: %d (%x)\n", __func__, chg_current, chg_current);
	...
	if (chg_current >=900) {
		...
		pr_err("%s : Da_G error charging current %d (%x), charge mode %d (%x)\n", __func__, chg_current, chg_current, chip->chg_mode, chip->chg_mode);
		return -EINVAL;
	}
	
	pr_err("%s : Da_G charging current %d (%x), charge mode %d (%x)\n", __func__, chg_current, chg_current, chip->chg_mode, chip->chg_mode);
					...
					int fast_current)
{
	...
	set_val -= 5;
	pr_err("%s : Da_G requesting fast current %d, (%x), input %d, (%x)\n", __func__, set_val, set_val, fast_current, fast_current);
	...
	if (val >= 0) {
		...
		data &= ~(0x7 << 5);
		data |= (set_val << 5);
		pr_err("%s : Da_G write reg %d, (%x), data %d, (%x)\n", __func__, reg, reg, data, data);
		...
		data = smb328a_read_reg(client, reg);
		pr_err("%s : Da_G read reg %d, (%x), data %d, (%x)\n", __func__, reg, reg, data, data);
		dev_info(&client->dev, "%s : => reg (0x%x) = 0x%x\n",
			__func__, reg, data);
	}
						...
						int chg_current)
{
	...
	if (chg_current < 500 || chg_current > 1200) {
		pr_err("%s : Da_G smb328a_adjust_charging_current outside bounds %d, (%x), ret=%d\n", __func__, chg_current, chg_current, ret);
		return -EINVAL;
	}

	chip->chg_set_current = chg_current;

	if ((chg_current > 450) && (chg_current < (ICL_1200mA+1))) {
		ret = smb328a_set_fast_current(client, chg_current);
		pr_err("%s : Da_G smb328a_set_fast_current %d, (%x), ret=%d\n", __func__, chg_current, chg_current, ret);
	}
	else
	{
		pr_err("%s : error! invalid setting current (%d)\n",
	...
	pr_err("%s : Da_G charging current %d, (%x)\n", __func__, chg_current, chg_current);
						...
						int chg_current)
{
	...
	pr_err("%s : Da_G input current requested: %d (%x)\n", __func__, chg_current, chg_current);

	if (chg_current < 450 || chg_current > 1200) {
		pr_err("%s : Da_G smb328a_set_input_current_limit outside bounds %d, (%x), ret=%d\n", __func__, chg_current, chg_current, ret);
		return -EINVAL;
	}
	...
	if ((chg_current > 450) && (chg_current < (ICL_1200mA+1))) {
		...
		pr_err("%s : Da_G smb328a_set_input_current_limit %d, (%x), ret=%d\n", __func__, chg_current, chg_current, ret);
	}
	else {
		pr_err("%s : error! invalid setting current (%d)\n",
				__func__, chg_current);
	...
	pr_err("%s : Da_G Input Current Limit %d, (%x)\n", __func__, chg_current, chg_current);
...
static int smb328a_get_input_current_limit(struct i2c_client *client)
{
	...
	if (val >= 0) {
		...
		if (data == 0) {
			pr_err("%s : Da_G ICL 450mA reg %d (%x) data is %d, (%x)\n", __func__, reg, reg, data, data);
			chip->chg_icl = ICL_450mA;
		}
		else {
			pr_err("%s : Da_G reg %d (%x) data is %d, (%x)\n", __func__, reg, reg, data, data);
	...
	pr_err("%s : Da_G Input Current Limit %d, (%x)\n", __func__, chip->chg_icl, chip->chg_icl);
...
static int smb328a_get_AICL_status(struct i2c_client *client)
{
	...
	if (val >= 0) {
		...
		if (data == 0) {
			...
			pr_err("%s : AICL 450mA %d (%x) Da_G reg %d (%x) data is %d, (%x)\n", __func__, chip->aicl_current, chip->aicl_current, reg, reg, data, data);
		}
		else if (data == 8) {
			chip->aicl_current = ICL_275mA;
			pr_err("%s : AICL 275mA %d (%x) Da_G reg %d (%x) data is %d, (%x)\n", __func__, chip->aicl_current, chip->aicl_current, reg, reg, data, data);
		}
		else
	...
	pr_err("%s : Da_G Automatic Input Current Limit %d, (%x)\n", __func__, chip->aicl_current, chip->aicl_current);
...
static int smb328a_enable_charging(struct i2c_client *client)
{
	...
	if (val >= 0) {
		...
		if (chip->chg_mode == CHG_MODE_AC ||
			...
			data = 0xb9;
		pr_err("%s : Da_G SMB328A_COMMAND set reg %d, (%x), value %d, (%x)\n", __func__, reg, reg, data, data);
		if (smb328a_write_reg(client, reg, data) < 0) {
			pr_err("%s : error!\n", __func__);
			return -EIO;
		}
		...
		data = smb328a_read_reg(client, reg);
		pr_err("%s : Da_G SMB328A_COMMAND read reg %d, (%x), value %d, (%x)\n", __func__, reg, reg, data, data);
		pr_info("%s : => reg (0x%x) = 0x%x\n", __func__, reg, data);
	}
...
static int smb328a_disable_charging(struct i2c_client *client)
{
	...
	if (val >= 0) {
		...
		dev_info(&client->dev, "%s : reg (0x%x) = 0x%x\n",
					__func__, reg, data);
		data = 0xb9;
		pr_err("%s : Da_G disabling charging set reg %d, (%x), value %d, (%x)\n", __func__, reg, reg, data, data);
		if (smb328a_write_reg(client, reg, data) < 0) {
			pr_err("%s : error!\n", __func__);
			return -EIO;
		}
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW: /* step1) Set charging current */
		pr_err("%s : Da_G PSPCN setting chg current %d, (%x)\n", __func__, val->intval, val->intval);
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL: /* step2) Set top-off current */
		...
		pr_err("%s : Da_G PSPCF setting top off chg current %d, (%x)\n", __func__, val->intval, val->intval);
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW: /* step3) Notify Vcell Now */
		...
		//pr_err("%s : Da_G PSPVN vcell level %d, (%x)=\n", __func__, val->intval, val->intval);
	...
	case POWER_SUPPLY_PROP_STATUS: /* step4) Enable/Disable charging */
		if (val->intval == POWER_SUPPLY_STATUS_CHARGING) {
			...
			pr_err("%s : Da_G PSPS enabling charging %d, (%x)=\n", __func__, ret, ret);
	...
	case POWER_SUPPLY_PROP_OTG:
		if (val->intval == POWER_SUPPLY_CAPACITY_OTG_ENABLE) {
			...
			pr_err("%s : Da_G PSPOTG enabling OTG %d, (%x)=\n", __func__, ret, ret);
		} else {
	...
	case POWER_SUPPLY_PROP_CURRENT_ADJ:
		...
		if (chip->chg_mode == CHG_MODE_AC) {
			ret = smb328a_adjust_charging_current(chip->client,
								val->intval);
			pr_err("%s : Da_G PSPCA enabling AC Charging current %d, (%x)=\n", __func__, val->intval, val->intval);
		} else {
			pr_info("%s : not AC mode,"
				" skip fast current adjusting\n", __func__);
			pr_err("%s : Da_G PSPCA not adjusting, not AC mode %d, (%x)=\n", __func__, ret, ret);
		}
		break;
	default:
		return -EINVAL;
...
static struct device_attribute sec_smb328a_attrs[] = {
	...
	SEC_SMB328A_ATTR(charge_current_override),
};
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	case SMB_READ_AICL:
		...
		break;
	case CHARGE_CURRENT_OVERRIDE:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			chip->charge_current_override);
		break;
	default:
		i = -EINVAL;
	}
			     ...
			     const char *buf, size_t count)
{
	...
	case SMB_WR_FAST:
		...
		break;
		case CHARGE_CURRENT_OVERRIDE: /* override charging current */
			if (sscanf(buf, "%d\n", &x) == 1) {
				if (x == 0) {
					chip->charge_current_override = 0;
				} else {
					chip->charge_current_override = x;			}
				ret = count;
			}
		break;
	default:
