...
// Da_G set max charging current without dangerous set
#define CHARGING_CURRENT_MAX_SAFE_MA 1000
...
struct sec_bat_info {
	...
	*/
	...
	unsigned int charge_current_override;
	unsigned int charge_enable_dangerous_settings;
	unsigned int charge_last_current_ma;
...
// Da_G override to AC if override enabled

			if (info->charge_current_override > 0) {
				pr_err("%s: Da_G USB Charge Override to AC 1A %d (%x)\n", __func__, info->cable_type, info->cable_type);
				info->cable_type = CABLE_TYPE_AC;
			} else {
...
static int sec_bat_enable_charging(struct sec_bat_info *info, bool enable)
{
	...
	if (enable) { /* Enable charging */
		...
		case CABLE_TYPE_USB:
			...
			pr_err("%s: Da_G PSSC USB charge cur set %d (%x)\n", __func__, val_chg_current.intval, val_chg_current.intval);
			break;
		case CABLE_TYPE_AC:
		...
		case CABLE_TYPE_UARTOFF:
			...
			pr_err("%s: Da_G PSSC 900mA charge cur set %d (%x)\n", __func__, val_chg_current.intval, val_chg_current.intval);
			break;
		case CABLE_TYPE_MISC:
			...
			info->full_cond_count = FULL_CHG_COND_COUNT;
			info->full_cond_voltage = FULL_CHARGE_COND_VOLTAGE;
			pr_err("%s: Da_G PSSC 700mA charge cur set %d (%x)\n", __func__, val_chg_current.intval, val_chg_current.intval);
			break;
		case CABLE_TYPE_UNKNOWN:
			...
			info->full_cond_count = USB_FULL_COND_COUNT;
			info->full_cond_voltage = USB_FULL_COND_VOLTAGE;
			pr_err("%s: Da_G PSSC 500mA charge cur set %d (%x)\n", __func__, val_chg_current.intval, val_chg_current.intval);
			break;
		default:
			dev_err(info->dev, "%s: Invalid func use\n", __func__);
			return -EINVAL;
		}
		pr_err("%s: Da_G cur requested %d (%x), override %d (%x)\n", __func__, val_chg_current.intval, val_chg_current.intval, info->charge_current_override, info->charge_current_override);
		if (info->charge_current_override != 0) {
			if (info->charge_enable_dangerous_settings == 1) {
				pr_err("%s: Da_G overriding charge current from %d (%x) to DANGEROUS VALUE %d %x\n", __func__, val_chg_current.intval, val_chg_current.intval, info->charge_current_override, info->charge_current_override);
				val_chg_current.intval = info->charge_current_override;
			}
			else {
				if ((info->charge_current_override > 0) && (info->charge_current_override < (CHARGING_CURRENT_MAX_SAFE_MA+1))) {
					pr_err("%s: Da_G overriding charge current from %d (%x) to %d %x\n", __func__, val_chg_current.intval, val_chg_current.intval, info->charge_current_override, info->charge_current_override);
					val_chg_current.intval = info->charge_current_override;
				}
				else {
					pr_err("%s: Da_G set enable_dangerous_settings to 1 to use this value!\n", __func__);
					pr_err("%s: Da_G attempted to change charge current from %d (%x) to %d %x\n", __func__, val_chg_current.intval, val_chg_current.intval, info->charge_current_override, info->charge_current_override);
				}
			}
		}
		pr_err("%s: Da_G last mA charging value was %d\n", __func__, info->charge_last_current_ma);
		info->charge_last_current_ma = val_chg_current.intval;
		/* Set charging current */
		ret = psy->set_property(psy, POWER_SUPPLY_PROP_CURRENT_NOW,
					&val_chg_current);
		pr_err("%s: Da_G PSPCN charge cur set %d (%x), ret %d (%x)\n", __func__, val_chg_current.intval, val_chg_current.intval, ret, ret);
		...
		ret = psy->set_property(psy, POWER_SUPPLY_PROP_CHARGE_FULL,
					&val_topoff);
		pr_err("%s: Da_G PSPCF charge cur set %d (%x), ret %d (%x)\n", __func__, val_topoff.intval, val_topoff.intval, ret, ret);
...
static struct device_attribute sec_battery_attrs[] = {
	SEC_BATTERY_ATTR(charge_last_current_ma),
	SEC_BATTERY_ATTR(charge_current_override),
	SEC_BATTERY_ATTR(charge_enable_dangerous_settings),
...
enum {
CHARGE_LAST_CURRENT_MA = 0,
CHARGE_CURRENT_OVERRIDE,
CHARGE_ENABLE_DANGEROUS_SETTINGS,
BATT_VOL,
BATT_SOC,
BATT_VFOCV,
BATT_TEMP,
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	switch (off) {
	case CHARGE_ENABLE_DANGEROUS_SETTINGS:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			info->charge_enable_dangerous_settings);
		break;
	case CHARGE_CURRENT_OVERRIDE:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			info->charge_current_override);
		break;
	case CHARGE_LAST_CURRENT_MA:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			info->charge_last_current_ma);
		break;
	case BATT_VOL:
			     ...
			     const char *buf, size_t count)
{
	...
	case CHARGE_CURRENT_OVERRIDE: /* override charging current */
		if (sscanf(buf, "%d\n", &x) == 1) {
			if (x == 0) {
				info->charge_current_override = 0;
			} else {
				info->charge_current_override = x;			}
			ret = count;
		}
		break;
	case CHARGE_ENABLE_DANGEROUS_SETTINGS: /* enable charging values higher than 1A */
		if (sscanf(buf, "%d\n", &x) == 1) {
			if (x == 0) {
				info->charge_enable_dangerous_settings = 0;
			} else {
				info->charge_enable_dangerous_settings = x;			}
			ret = count;
		}
		break;
