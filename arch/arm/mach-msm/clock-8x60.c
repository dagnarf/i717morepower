...
void clock8x60_set_sys_vdd(unsigned int level, int sys_vdd_uv) {
	...
	if (sys_vdd_uv % VDD_STEP != 0) {
		pr_err("Requested VDD not a valid step(multiple of %d): sys_vdd_uv: %d\n", (unsigned int)VDD_STEP, sys_vdd_uv);
		return;
	}
		
	for (i = 0; (vdd_uv[i]); i++) {
		if (level == 0)
			new_sys_vdd_uv = min(max(((unsigned int)vdd_uv[i] + sys_vdd_uv), (unsigned int)MIN_SYS_VDD), (unsigned int)MAX_SYS_VDD);
		else if (i == (level - 1))
			new_sys_vdd_uv = min(max((unsigned int)sys_vdd_uv, (unsigned int)MIN_SYS_VDD), (unsigned int)MAX_SYS_VDD);
		else 
			continue;

		vdd_uv[i] = new_sys_vdd_uv;
